...
namespace MSR.LST.ConferenceXP
{
    ...
    internal class ThumbnailListView: Panel
    {
        ...
        internal void InsertThumbnail( Image image, string slideTitle, int index, Guid tag )
        {
            lock (this)
            {
                if( index < 0 || picBoxes.Count < index )
                    throw new ArgumentOutOfRangeException("Index must be within the collection.");
        ...
        {
            Debug.WriteLine("Location of pb changed to: "+((PictureBox)sender).Location);
        }
