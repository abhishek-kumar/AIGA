...
namespace MSR.LST.ConferenceXP
{
    ...
    public class PresentationCapabilityFMain : CapabilityForm
    {
        ...
        private void InitializeComponent()
        {
            ...
            this.pnlInk.Size = new System.Drawing.Size(216, 46);
            ...
            this.inkToolBar.Size = new System.Drawing.Size(216, 96);
        ...
        //        private ICapabilitySender wmvCapability = null;
        //        private bool videoStopped = true;
        //        private const string WMV_CAPABILITY_NAME = "Windows Media Video";
        ...
        private void RTStrokeReceived(RTStroke rtStroke)
        {
            ...
            Log("Stroke ID: rtStroke.StrokeIdentifier.ToString()=" 
        ...
        private void PageReceived(Page page)
        {
            ...
            {
                statusBar.StatusMessage = "Receiving Page:  "
                    + (pageReceivedCounter+1) + " / "
                    + rtDocument.Organization.TableOfContents.Count.ToString(); 
            }
            else
            {
            ...
            Log("I just added a Page object with the id: " + page.Identifier + ",");
            Log("The TOC Node identifier is: " + rtDocumentHelper.TOCNodeIdentifierToPageIdentifier(page.Identifier) + ",");
        ...
        private void RTDocumentReceived(RTDocument rtDocReceived)
        {
            ...
            if( (rtDocument == null) || (!rtDocReceived.Identifier.Equals(rtDocument.Identifier)) )
            {
                ...
                Log("rtDocID in RTDocumentReceived method: " + rtDocument.Identifier.ToString());
                ...
                statusBar.StatusMessage = "Receiving Page: 1 / "
                    + rtDocument.Organization.TableOfContents.Count.ToString(); 
        ...
        private void ShowPage(Guid tocNodeIdentifier)
        {
            ...
            {
                Log("rtDocument == null,");
                throw new Exception("rtDocument == null");
            }
            ...
            if (pageIdentifier == Guid.Empty)
            {
                ...
                throw new Exception("Unable to find Page for current TOCNode");
            }
            ...
            {
                Image tempImg = pbRTDoc.Image;
                pbRTDoc.Image = ThumbnailListView.GenerateBitmapAlertText("Page Not Found!");
                if( tempImg != null )
                    tempImg.Dispose();
                return;
            }
            ...
            Log("start displaying the page id: " + p.Identifier);
        ...
        private void inkOverlay_Stroke(object sender, InkCollectorStrokeEventArgs e)
        {
            if (e.Stroke.DrawingAttributes.Transparency != 255)
            {
                ...
                Log("RatioWidthSend=" + ratioWidthSend.ToString() 
                    + "RatioHeightSend=" + ratioHeightSend.ToString() + ",");
        ...
        private void DocumentNotLoadedMessage()
        {
            MessageBox.Show(
                this, 
                "You must first open a document.", 
                "Error", 
        ...
        {
            Log("rtDoc.Identifier: " + rtDocument.Identifier + ",");
            Log("rtDoc.Metadata.Title: " + rtDocument.Metadata.Title + ",");
            Log("rtDoc.Organization.TableOfContents.Count: " + rtDocument.Organization.TableOfContents.Count + ",");
            Log("rtDoc.Resource.Pages.Count: " + rtDocument.Resources.Pages.Count + ",");
        }
        ...
        private void DisplayPageInfo()
        {
            ...
            {
                Log("PAGE_ID: " + p.Identifier.ToString() + ",");
            }
        }
        ...
        private void DisplayRTDocumentStructureInfo()
        {
            ...
            {
                Log("TOC_ID[" + i + "]: " + rtDocument.Organization.TableOfContents[i].Identifier.ToString() + ",");
                Log("rtDocumentHelper.TOCNodeIdentifierToPageIdentifier(TOC_ID): " +
                    rtDocumentHelper.TOCNodeIdentifierToPageIdentifier(rtDocument.Organization.TableOfContents[i].Identifier) + ",");
            }
            DisplayPageInfo();
        ...
        {
            // TODO: Decide on debug for ship & delete commented lines below
            Log("Size Info: pbRTDoc.Width = " 
                + pbRTDoc.Width.ToString()
                //                " rtDoc.Rsc.Pages[x].Img.Width = " 
                //                + rtDocument.Resources.Pages[pageShowing].Image.Width.ToString()
                + " bRTDoc.Height = " + pbRTDoc.Height.ToString() 
                //                + " rtDoc.Rsc.Pages[x].Img.Height = " 
                //                + rtDocument.Resources.Pages[pageShowing].Image.Height 
                + ",");
        }
        ...
        private void UpdateCurrentPage()
        {
            ...
            Log(DateTime.Now.ToString() + "-> Page ID sent: " + p.Identifier.ToString() + ",");
        ...
        private void navigateToIndex(int navToIndex, bool synchronize)
        {
                ...
                && (navToIndex != index) )
            {
                ...
                catch (Exception ex)
                {
                    ...
                    MessageBox.Show(
                        this,
                        "An error occured during navigation, the page might be absent or might be still in the download process",
                        "Error",
                        MessageBoxButtons.OK,
                        MessageBoxIcon.Error);
                }
        ...
        private void openFile()
        {
            ...
            try
            {
                if (dr == DialogResult.OK)
                {
                    ...
                    if (extension == ".ppt" || extension == ".pptx")
                    {
                        ...
                        lock(this)
                        {
                            statusBar.SetWaitStatusMessage("Importing PowerPoint document...");
                    ...
                    else if (extension == ".rtd")
                    {
                        ...
                        lock(this)
                        {
                            ...
                            {
                                MessageBox.Show("Error in opening " + fileName + "\r\nVersion may be out of date");
                                return;
                            }
                        }
                        #endregion
                    ...
                    Log("rtDocID = " + rtDocument.Identifier.ToString());
                    ...
                    {
                        EnableSlideSend(false);
                        statusBar.StatusMessage = "Sending slides...";
            ...
            {
                MessageBox.Show("Error in opening " + cdlg.FileName + ", error message: " + ex.Message.ToString());
                return;
            }
        }
        ...
        private void resendSlides()
        {
            ...
            {
                EnableSlideSend(false);
                statusBar.StatusMessage = "Resending slides...";
            ...
            {
                MessageBox.Show("Error during communication: " + ex.Message.ToString());
            }
        }
        ...
        private void InsertSnapshot()
        {
            if( window.Minimized )
            {
                MessageBox.Show("Snapshots of minimized windows cannot be taken." + Environment.NewLine
                    + "Please restore or maximize the window.");
                return;
            }
            ...
            {
                MessageBox.Show("Error while taking snapshot."
                    + Environment.NewLine + "Your target snapshot window may be in an invalid state or closed.");
                return;
            }
            ms.Position = 0;
            ...
            TOCNode tn = rtDocumentHelper.SendPageInsert(page, "Snapshot of " + window.Text, index);
        ...
        private void InsertWhiteboard()
        {
            ...
            TOCNode tn = rtDocumentHelper.SendPageInsert(page, "Whiteboard " + (index+2), index);
        ...
        private void ScaleAndPositionPbImage()
        {
            ...
            Log("New ratio for pbRTDoc = " + newRatio.ToString());
        ...
        {
            EnableSlideSend(false);
            statusBar.StatusMessage = "Resending current slide...";
            ...
            if (mode == InkToolBarMode.Pen)
            {
                ...
                {         
                    ...
                    case "Yellow":            
                        ...
                        break;           
                    default:
                        throw new Exception(string.Format("Unexpected pen color '{0}'", color.ToString()));
                }
            }
            else if (mode == InkToolBarMode.Highlighter)
            {
                ...
                {         
                    ...
                    case "Blue":            
                        ...
                        break;              
                    default:
                        throw new Exception(string.Format("Unexpected highlighter color '{0}'", color.ToString()));
                }
            } 
        ...
        private void miHelpAbout_Click(object sender, System.EventArgs e)
        {
            ...
            MessageBox.Show
                ...
                "Presentation : " + fvi.FileVersion + "\n" +
                Conference.About,
                "ConferenceXP Presentation"
                );
        }
        ...
        private void miOpenRemote_Click(object sender, System.EventArgs e)
        {
            ...
            try
            {
                if (dr == DialogResult.OK)
                {
                    ...
                    if (System.IO.Path.GetExtension(fileName).ToLower() == ".rtd")
                    {
                        // Open a RTD file
                        lock(this)
                        {
        ...
        public override void AddCapability(ICapability capability)
        {
            ...
            if(presentationCapability == null)
            {
                ...
                // Init StatusBar
                statusBar.SetBusyStatusMessage("Loading...");
                ...
                rtDocument.Metadata.Title = "Whiteboard Session";
