    ...
    public class frmWMFile : CapabilityForm
    {
        ...
        private void btnSelect_Click(object sender, System.EventArgs e)
        {
            ...
            if (ofd.ShowDialog(this) == DialogResult.OK)
            {
                ...
                if (file != null)
                {
                    string exn = System.IO.Path.GetExtension(file).ToLower();

                    if (exn != ".wmv" && exn != ".wma")
                    {
                        MessageBox.Show(string.Format("Unexpected file extension - {0}, for file " +
                            "{1}.  We only support .wmv and .wma for now.", exn, file));
                    ...
                    else
                    {
                        ...
                        lblInfo.Text = "Selected file: " + file;
                    }
                }
