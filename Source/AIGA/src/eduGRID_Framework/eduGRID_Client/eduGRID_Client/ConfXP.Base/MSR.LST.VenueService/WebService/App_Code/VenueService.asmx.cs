...
namespace MSR.LST.ConferenceXP.VenueService
{
    ...
    public class VenueService : System.Web.Services.WebService
    {
        ...
        /// <param name="assemblyName">The file version of the entry assembly on the
        /// client machine as a string.</param>
        /// <param name="errorMessage">A user-readable error message explaing the
        /// nature of the error and the expected assembly version.</param>
        ...
        public bool CheckAssemblyVersion(string assemblyVersion, out string errorMessage)
        {
            ...
            if( version != null && version != String.Empty )
            {
                ...
                {
                    errorMessage = ConfigurationManager.AppSettings["VersionErrorMsg"];
                }
            }
        ...
        {
            return System.Configuration.ConfigurationManager.AppSettings["PrivacyPolicyURL"];
        }
        #endregion
        ...
        private static void ValidateParticipant( Participant p )
        {
            ...
            if ( p.Name != null && p.Name.Length > 255 )
                throw new ArgumentException("Name must be between 1 and 255 characters long", p.Name );

            if ( p.Email != null && p.Email.Length > 0 )
                ValidateEmail(p.Email);

            // Set the max image size to 96x96 pixels at 32-bit color depth
            if ( p.Icon != null && p.Icon.Length > 36864 )
                throw new ArgumentException("Participant icon must be less than 36 KB (a 96x96 pixel image).");
        }
        ...
        private static void ValidateEmail( string email )
        {
            ...
            if ( !reg.IsMatch(email) )
                throw new ArgumentException(email + " does not appear to be a valid email address, sorry");
        }
        #endregion
