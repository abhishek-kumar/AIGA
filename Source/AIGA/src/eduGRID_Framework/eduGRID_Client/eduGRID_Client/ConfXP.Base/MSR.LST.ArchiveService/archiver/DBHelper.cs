    ...
    public class DBHelper
    {
        ...
        static public bool RenameConference( int conferenceID, string newName )
        {
            if( newName.IndexOfAny( new char[]{'\'', '\"'} ) >= 0 )
                throw new ArgumentException("Conference name cannot contain ''' or '\"'.");

            string command = "UPDATE Conference SET description = '" + newName + "' WHERE conference_id = " + conferenceID;
        ...
        static internal void SaveBufferAndIndices( int streamID, Index[] indices, int indicesCount, byte[] buffer )
        {
            ...
            {
                eventLog.WriteEntry("Failed to save all indices in BufferRecorder::SaveIndices. Should have saved " 
                    + indicesCount + " only saved " + affected, EventLogEntryType.Error, ArchiveServiceEventLog.ID.IndiciesFailedToSave );
            }
            #endregion
        }
        #endregion
