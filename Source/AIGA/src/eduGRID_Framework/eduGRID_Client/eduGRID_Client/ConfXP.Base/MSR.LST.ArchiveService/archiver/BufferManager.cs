...
namespace MSR.LST.ConferenceXP.ArchiveService
{
    ...
    internal class BufferManager : IDisposable
    {
        ...
        {
            eventLog = new ArchiveServiceEventLog( ArchiveServiceEventLog.Source.BufferManager);
        }
        #endregion
        ...
        public void AddFrame( FrameWithTicks frame )
        {
            lock(this)
            {
                ...
                // NOTE: this breaks from one BufferManager to another.  If the last frame in this buffer and the first frame in the next 
        ...
        public void EnableWrite()
        {
            lock(this)
            {
                ...
                if( this.isAvailable )
                {
                    Trace.WriteLine(String.Format("CanSave set on stream {0} at {1}", streamID, DateTime.Now.Ticks));
            ...
            lock( this )
            {
                ...
                try
                {
                    ...
                    if( currentIndex > 0 )
                    {
                        ...
                        Trace.WriteLine(String.Format("SBAI called on stream {0} at {1}", streamID, DateTime.Now.Ticks));
                        ...
                        Console.WriteLine("SBAI took: "+(takenTime/Constants.TicksPerMs)+" ms");
#endif
                    }
                }
                ...
                // if we didn't overflow, say that we wrote the data.  if we did overflow, we don't want to lose the data
                if( !overflowed )
                {
                    bufferIndex = 0;
                    currentIndex = 0;
