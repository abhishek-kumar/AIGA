...
namespace MSR.LST.ConferenceXP.ArchiveService
{
    ...
    internal class BufferPlayer
    {
        ...
        private void InitiatePopulation( object o, bool timedOut )
        {
            ...
            if ( !timedOut )
            {
                if( populating == false )
                {
                    ...
                    eventLog.WriteEntry("Populate called on sender that is not ready", EventLogEntryType.Error, 
        ...
        public void Populate( long newStartingTicks )
        {
            ...
            lock( populatingLockObj )
            {
                ...
                if ( indexCount > 0 )
                {
                    ...
                    Trace.WriteLine( String.Format("Loaded buffer (ID: {0}) with: {1} sec, {2} bytes, {3} frames", streamID, 
                        ((double)(endingTick - startingTick) / 10000000.0), (indices[indexCount -1].end - indices[0].start + 1), indexCount) );
                }
                else
                {
                ...
                Trace.WriteLine("TIMING: Load buffer took: "+(takenTime/Constants.TicksPerMs)+" ms");
#endif
        ...
        {
            if( this.populating )
                throw new InvalidOperationException("BufferPlayer::SendFrames called while Populating.");
                ...
                while ( currentIndex < indexCount && indices[currentIndex].timestamp <= timeBoundary )
                {
                    ...
                    if ( frameLength > 0)
                    {
                        ...
                        if( lateness > Constants.TicksSpent )
                            Trace.WriteLine(String.Format("--- FRAME LATENESS OF: {0} ms", lateness));
                    ...
                    {
                        // (pbristow) Why in the nuts would this happen???
                    ...
                    if( takenTime > Constants.TicksSpent )
                        Trace.WriteLine(String.Format("TIME WASTED TO SEND A FRAME: {0} ms, ID: {1}, bytes: {2}",
        ...
        { 
            ...
            {
                if( this.populating )
                    throw new InvalidOperationException("LastTicks is transient while populating & thus cannot be read.");
        ...
        { get { return new DateTime(LastTick).ToString("T"); } }

        // Also for debugging
        public string FirstTime
        { get { return new DateTime(FirstTick).ToString("T"); } }
