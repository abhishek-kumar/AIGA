    ...
    internal class BufferRecorder : IDisposable
    {
        ...
        public void EnableWrite()
        {
            lock(this)
            {
                ...
                if( this.isAvailable )
                {
                    Trace.WriteLine(String.Format("BufferRecorder::EnableWrite called on buffer {0}, stream {1} at {2}", 
        ...
        private void Flush()
        {
            Trace.WriteLine(String.Format("BufferRecorder::Flush called on buffer {0}, stream {1}.", number, streamID));
        ...
        {
            Trace.WriteLine(String.Format("BufferRecorder::InitiateSave called on buffer {0}, stream {1}.", number, streamID));
