...
namespace MSR.LST.MDShow
{
    ...
    public struct RECT
    {
        ...
        public override string ToString()
        {
            return string.Format(CultureInfo.CurrentCulture, 
                "L: {0}, T: {1}, R: {2}, B: {3}", left, top, right, bottom);
        }
    ...
    public struct SIZE
    {
        ...
        public override string ToString()
        {
            return string.Format(CultureInfo.CurrentCulture, "{0}x{1}", cx, cy);
        }
    ...
    public struct MPEG1VIDEOINFO
    {
        ...
        {
            return string.Format(CultureInfo.CurrentCulture,
                "MPEG1VIDEOINFO - StartTimeCode: {0}  SequenceHeaderByteCount: {1}  " +
                "SequenceHeader: {2}\r\n{3}",
    ...
    public struct MPEG2VIDEOINFO
    {
        ...
        {
            return string.Format(CultureInfo.CurrentCulture,
                "MPEG2VIDEOINFO - StartTimeCode: {0}  SequenceHeaderByteCount: {1}  " +
                "Profile: {2}  Level: {3}  Flags: {4}  SequenceHeader: {5}\r\n{6}",
                StartTimeCode, cbSequenceHeader, Profile, Level,
                Flags, SequenceHeader.ToString(), hdr);
        }
    }
    /// <summary>
    /// Located in Platform SDK\include\Amvideo.h
    ...
    public struct VIDEOINFOHEADER2
    {
        ...
        {
            return string.Format(CultureInfo.CurrentCulture,
                "VIDEOINFOHEADER2 - Source: {0}  Target: {1}  BitRate: {2}  " +
                "BitErrorRate: {3}  AvgTimePerFrame: {4}  InterlaceFlags: {5}  " +
                "CopytProtectFlags: {6}  PictAspectRatioX: {7}  PictAspectRatioY: {8}  " +
                "Reserved1: {9}  Reserved2: {10}\r\n{11}",

                Source.ToString(), Target.ToString(), BitRate, 
                BitErrorRate, AvgTimePerFrame, InterlaceFlags,
                CopyProtectFlags, PictAspectRatioX, PictAspectRatioY,
                Reserved1, Reserved2, BitmapInfo.ToString());
        }
    }
    ...
    public struct BITMAPINFOHEADER
    {
        ...
        {
            return string.Format(CultureInfo.CurrentCulture,
                "BITMAPINFOHEADER - Size: {0},  Width: {1},  " +
                "Height: {2},  Planes: {3},  BitCount: {4},  Compression: {5},  " +
                "SizeImage: {6},  XPelsPermeter: {7},  YPelsPerMeter: {8},  " +
                "ClrUsed: {9},  ClrImportant: {10}",
    ...
    public struct WAVEFORMATEX
    {
        ...
        {
            return string.Format(CultureInfo.CurrentCulture,
                "WAVEFORMATEX -  FormatTag: {0}\tChannels: {1}\tSamplesPerSec: {2}\t" +
                "AvgBytesPerSec: {3}\tBlockAlign: {4}\tBitsPerSample: {5}\tSize: {6}",
    ...
    public struct AUDIO_STREAM_CONFIG_CAPS
    {
        ...
        {
            return string.Format(CultureInfo.CurrentCulture,
                "AUDIO_STREAM_CONFIG_CAPS -  guid: {0},  " +
                "MinimumChannels: {1},  MaximumChannels: {2},  " +
                "ChannelsGranularity: {3},  " +
                "MinimumBitsPerSample: {4},  MaximumBitsPerSample: {5},  " +
                "BitsPerSampleGranularity: {6},  " +
                "MinimumSampleFrequency: {7},  MaximumSampleFrequency: {8},  " +
                "SampleFrequencyGranularity: {9}",

                // Docs clearly say this is a major type
                MediaType.MajorType.GuidToString(guid),
                MinimumChannels, MaximumChannels,
                ChannelsGranularity,
                MinimumBitsPerSample, MaximumBitsPerSample,
