...
namespace MSR.LST.ConferenceXP
{
    ...
    public class VideoCapability : CapabilityDeviceWithWindow, ICapabilitySender, ICapabilityViewer, ICapabilityWindowExtended, ICapabilityWindowAutoSize
    {
        ...
        /// <summary>
        /// Get the VideoCaptureGraph for this capability
        /// </summary>
        public VideoCaptureGraph VideoCaptureGraph
        {
            get{return (VideoCaptureGraph)cg;}
        }
        ...
        public void ShowCameraConfiguration()
        {
            if(vcg.VideoSource.IsVfw)
            {
                System.Windows.Forms.MessageBox.Show(form, 
                    "Can not configure VFW driver-based cameras when in a venue");
                return;
            }

            if (vcg != null && vcg.VideoSource.HasSourceDialog)
            {
        ...
        private void RestoreVideoSettings()
        {
            ...
            if(bytes != null)
            {
                ...
                if(mt.cbFormat != 0)
                {
                    ...
                    {
                        // Set and free
                        vcg.Source.SetMediaType(ref mt);
                    }
